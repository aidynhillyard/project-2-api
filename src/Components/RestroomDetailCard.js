...
const RestroomDetailCard = ({ handleAddRestroomList, restrooms, restroomId }) => {
    ...
    const restroomList = restrooms?.map((soloRestroom, id) => (
            <div className="restroom-detail-card" key={id} style={{ display: (restroomId === id) ? "flex" : "none" }}>
                ...
                <button className="purple-button" onClick={() => handleAddRestroomList(soloRestroom?.name, soloRestroom?.street, soloRestroom?.city, soloRestroom?.state, soloRestroom?.directions, soloRestroom?.comment, soloRestroom?.accessible, soloRestroom?.unisex, soloRestroom?.changing_table)}>Add to List</button>
            </div>
        ))
